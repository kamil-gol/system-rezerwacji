...
export const updateReservation = async (req: AuthRequest, res: Response) => {
  ...
  Object.keys(data).forEach(key => {
    if (!['pricePerPerson', 'totalPrice', 'depositAmount', 'finalAmount', 'numberOfGuests', 'durationHours'].includes(key)) {
  ...
  if (data.pricePerPerson !== undefined) {
    updateData.pricePerPerson = data.pricePerPerson ? new Prisma.Decimal(data.pricePerPerson) : null;
  }
  if (data.totalPrice !== undefined) {
    updateData.totalPrice = data.totalPrice ? new Prisma.Decimal(data.totalPrice) : null;
  }
  if (data.depositAmount !== undefined) {
    updateData.depositAmount = data.depositAmount ? new Prisma.Decimal(data.depositAmount) : null;
  }
  
  // Recalculate final amount if pricing fields changed
  if (data.numberOfGuests !== undefined || data.pricePerPerson !== undefined || data.totalPrice !== undefined || data.pricingType !== undefined) {
    const calculatedAmount = calculateFinalAmount({
      pricingType: data.pricingType !== undefined ? data.pricingType : existing.pricingType,
      numberOfGuests: data.numberOfGuests !== undefined ? data.numberOfGuests : existing.numberOfGuests,
  ...
  if (data.durationHours !== undefined) {
    ...
    updateData.autoGeneratedNotes = generateExtraHoursNote(data.durationHours);
  }

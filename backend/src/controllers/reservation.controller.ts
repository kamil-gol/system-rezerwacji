...
export const updateReservation = async (req: AuthRequest, res: Response) => {
  ...
  let finalAmount: Prisma.Decimal = existing.finalAmount;
  if (data.numberOfGuests || data.pricePerPerson || data.totalPrice) {
    const calculatedAmount = calculateFinalAmount({
      ...
      pricePerPerson: data.pricePerPerson || (existing.pricePerPerson ? existing.pricePerPerson.toNumber() : 0),
      totalPrice: data.totalPrice || (existing.totalPrice ? existing.totalPrice.toNumber() : 0)
    });
